...
const PayWithFlutterwaveScreen = () => {
  ...
  const navigateToFlutterwaveOnboarding = async (data:any) => {
    try {
      ...
      console.log(body)
      const response = await axios.post('coinnet/fiat/initialize', body);
      ...
      setTimeout(() => {
        ...
        if(err.response.data.message === 'Insufficient Balance in fiat'){
            ToastAndroid.show('Insufficient funds! Fund your fiat', ToastAndroid.SHORT);
      ...
      <SafeAreaView style={[styles.container, {backgroundColor:theme ? '#0F0F0F': 'white'}]}>
        ...
        <KeyboardAvoidingView style={styles.mainContent} behavior="padding" keyboardVerticalOffset={100}>
          <ScrollView contentContainerStyle={styles.scrollContainer}>
            <ThemedText style={styles.title}>Deposit with Coinnet</ThemedText>
